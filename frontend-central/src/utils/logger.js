/**
 * Frontend Central Logger
 *
 * Logger utility for capturing and sending important actions/errors to backend
 * Logs are stored in backend/logs/frontend-central/ folder
 */

const API_BASE_URL =
  import.meta.env.VITE_API_BASE_URL || "http://localhost:3000";
const SOURCE = "frontend-central";
...
const getUserInfo = () => {
  const token =
    localStorage.getItem("central_token") ||
    sessionStorage.getItem("central_token");
  const role = localStorage.getItem("role") || sessionStorage.getItem("role");
  const userId =
    localStorage.getItem("user_id") || sessionStorage.getItem("user_id");

  return { userId, role, hasToken: !!token };
};
...
const logger = {
   ...
   */
  logTenant(action, details = {}) {
    queueLog(LOG_LEVELS.INFO, `Tenant: ${action}`, { action, ...details });
  },
  ...
  logUser(action, details = {}) {
    queueLog(LOG_LEVELS.INFO, `User: ${action}`, { action, ...details });
  },

  /**
   * Log package management operations
   */
  logPackage(action, details = {}) {
    queueLog(LOG_LEVELS.INFO, `Package: ${action}`, { action, ...details });
  },
   ...
   */
  logMonitor(action, details = {}) {
    queueLog(LOG_LEVELS.INFO, `Monitor: ${action}`, { action, ...details });
  },
  ...
  logExport(action, details = {}) {
    queueLog(LOG_LEVELS.INFO, `Export: ${action}`, { action, ...details });
  },
